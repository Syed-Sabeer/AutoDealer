...
@section('title', __('Edit Car Fuel Type'))
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.fuel-types.update', $carFuelType->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row p-5">
                        <h3>{{ __('Edit Car Fuel Type') }}</h3>
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                                ...
                                value="{{ old('name', $carFuelType->name) }}" />
                            @error('name')
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('slug') is-invalid @enderror" type="text" id="slug"
                                ...
                                value="{{ old('slug', $carFuelType->slug) }}" />
                            @error('slug')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            @enderror
                            @if($carFuelType->image)
                                <img src="{{ asset($carFuelType->image) }}" alt="Image" class="mt-2" width="120">
                            @endif
                        </div>
                        <div class="col-md-6 mb-4">
                            <label class="switch switch-square">
                                ...
                                <input type="checkbox" class="switch-input @error('is_featured') is-invalid @enderror" id="is_featured"
                                    name="is_featured" {{ old('is_featured', $carFuelType->is_featured) == '1' ? 'checked' : '' }}/>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                placeholder="{{ __('Enter car fuel type description') }}" cols="10" rows="5">{{ old('description', $carFuelType->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Update Car Fuel Type') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
